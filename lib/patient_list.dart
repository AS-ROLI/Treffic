...
class _patient_listState extends State<patient_list> {
  ...
  Future getCurrentUserAndHospitalID() async {
    try {
      ...
      final message =
      await _firestore.collection("Authentication").get();
  ...
  Future DeleteTempId() async {
    try {
      final message =
      await _firestore.collection("Temporary_info").get();
      for (var messages in message.docs) {
        if (messages.data()["email"] == loggedInUser.email) {
          _firestore
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  ...
                  for (var message in messages1!) {
                    ...
                    if ((message.data() as  Map<String, dynamic>)!["information"][2] == hospitalid) {
                      print("hi there");
                      final PatientName = (message.data() as  Map<String, dynamic>)!["information"][0];
                      final PatientAge = (message.data() as  Map<String, dynamic>)!["information"][4];
